...
def create_app(data_dir: str | Path | None = None, *, model=None, seed_demo: bool = True) -> FastAPI:
    directory = Path(data_dir or os.getenv("BAHITH_DATA_DIR", str(ROOT / ".bahith")))
    templates = Jinja2Templates(directory=ROOT / "templates")
    templates.env.globals["label_ar"] = lambda category: CATEGORY_LABELS_AR.get(category, category)

    @asynccontextmanager
    async def lifespan(application):
        application.state.store = DocumentStore(directory, load_corpus() if seed_demo else None)
    ...
    async def response_headers(request: Request, call_next):
        ...
        # no-referrer turns browser form-navigation Origin into null; same-origin
        # keeps native forms valid while withholding query URLs from external sites.
    ...
    def render(request: Request, template: str, context: dict | None = None, status_code: int = 200):
        base = {"csrf_token": request.app.state.csrf_token, "documents": request.app.state.store.documents(),
    ...
    def searcher(request: Request):
        ...
        if state.searcher is None or state.generation != generation:
            ...
            state.searcher = ArabicSearcher(state.store.passages(), model=old_model,
                                           cache_dir=directory / "embeddings", load_embeddings=False)
